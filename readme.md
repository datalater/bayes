...
+ `사실 1` : 눈 앞에 단지 A와 단지 B가 있다.
+ `사실 2 `: A에는 대체로 흰 공이 들어있다
+ `사실 3` : B에는 대체로 검은 공이 들어있다
+ `사실 4` : 검은 공이 나왔다

검은 공이 나온 단지는 A일까, B일까?
...
따라서 지금 내린 판단인 '단지 B다'가 맞는지 틀리는지 그 자체는 알 수 없지만, **이 방법으로 계속 추정해 나가면 불과 10%의 확률이기는 하나 잘못된 결론을 내리게 된다**.
즉 '단지 A임에도 단지 B라고 결론짓는' 일이 발생한다.
...
### 네이만-피어슨 식 가설검정을 단지 문제에 적용하기

+ 귀무가설은 '단지 A다'이다.
+ 대립가설은 당연히 '단지 B다'가 된다.
+ 유의수준 alpha를 0.1로 설정하면 단지 A로부터 검은 공이 나온느 것을 관측할 확률은 alpha가 된다.
...
> **Note:** 네이만과 피어슨은 지금의 통계학 형태를 완성한 통계학자들이다. 또 한 사람 중요한 공헌을 한 피셔라는 통계학자가 있어서 피셔-네이만-피어슨 통계학이라 부ㅡ기도 한다.
