...
사건 A가 N회 관측하여 x회 일어난 경우, 우리는 사건 A의 확률을 x/N이라고 추정한다.
여기에는 최우원리가 숨어있다.
x/N이라고 추정하는 것이 다른 추정치보다 가장 확률이 크기 때문이다.
가장 확률이 큰 것으로 결론을 내리는 것을 '최우원리'라고 한다.

이러한 '최우원리'는 표준 통계학과 베이즈통계학에서 사용되는 공통된 발상이다.
...
예컨대 누군가 어디에 물건을 두고 와 잃어버렸는데, 그것은 A나 B 중 어느 한 사람이라고 치자.
A씨는 물건을 자주 잃어버리는 사람이며 B씨는 그런 일이 거의 없는 사람이다.
이때 대개는 물건을 잃어버린 쪽이 A씨일 것이라고 추론할 것이다.

이렇듯 '**일어날 확률이 더 큰 쪽이 원인일 것이라고 생각하는 사고**'를 '**최우원리**'라고 한다.

이처럼 최우원리는 우리에게 매우 익숙한 사고법이며 많은 학문 분야에 이용되어 왔다.
특히 물리학 중에서도 통계물리학 분야에서 뚜렷이 드러난다.
통계물리에서는 최우원리를 이용해 다양한 물리 현상을 해명하고 있다.

### 최우원리에 근거하고 있는 베이즈 추정

베이즈 추정에서 사후확률은 `(사전확률) * (조건부 확률)`에 비례한다.
따라서 사전확률이 크거나 조건부 확률이 큰 쪽이 원인으로 선택되기가 쉽다.
이는 그야말로 최우원리 그 자체다.

### 네이만-피어슨 통계학도 최우원리에 근거하고 있다

표준 통계학에서는 최우원리가 '**통계적 추정을 입증**'하는 데 사용된다.

'통계적 추정의 입증'이란 통계학에서 무언가를 추정할 때 '**왜 그렇게 생각하는가**', '**그렇게 생각하는 것이 어떤 이점을 가져다주는가**'를 설명하는 것이다.

여기서는 '**점추정**'이라 불리는 통계적 추정을 예로 알아보자.

가령 '손님의 총 인원이 100명을 넘는' 현상을 생각해보자.
현상이 일어날 확률을 p라고 한다.
당연히 일어나지 않을 확률은 1-p가 된다.
이 현상을 10일 동안 관측한 결과 10일 중 4일간 일어났고, 나머지 6일간은 일어나지 않았다.
이때 확률 p는 몇이라고 추정할 수 있을까?

아무래도 '10일 중 4일 일어났으니 p = 4/10 = 0.4일 것이다'라고 추정하는 것이 가장 일반적이다.
이것은 통계학의 입장에서 '일어난 횟수의 평균치'를 구해 그것을 p라는 추정치로 만든 것과 동일하다.

의문점은 '왜 일어난 횟수의 평균치를 현상이 일어날 확률 p의 추정치로 잡는가'이다.
잘 생각해 보면 '몇 번 중에 몇 번 일어났다'는 사실과 '일어날 확률'이라는 것이 직접적으로 연결되어 있지는 않다.
사실은 이를 입증하는데 최우원리가 사용된 것이다.

여기서 일어날 확률이 p인 현상에 대해 '이 현상이 10회 중 4회 일어날 확률' L을 '**이항정리**'를 사용해서 p의 식으로 나타내 보자.

```
10 choose 4 * p^4 * (1-p)^6
```

확률 p값을 변화시켜나갈 때 확률 L의 값이 몇인지 프로그램으로 계산해보자.
확률 p를 가로축으로, 확률 L의 수치를 세로축에 두고 위 관계를 그래프로 만들어 보자.

![p.103 도표8-1]()

예컨대 p=0.2일 때 L=0.088이다.
p=0.4일 때 L값이 가장 커진다는 것을 알 수 있다.
즉, 평균치인 0.4를 p로 설정한 경우 확률 L이 가장 커지는 것이다.
이에 따라 통상 통계적 추정에서는 p=0.4라고 추정한다.
그리고 0.4를 p의 '**최우추정량**'이라 부른다.

### 최우추정량과 평균치의 관계

최우추정량이 평균치가 되는 것은 위 사례에서만 나타나는 우연이 아니다.
N회 관측하여 x회 일어난 경우 최우추정량이 x/N이 된다는 사실은 미분법을 통해 간단히 증명할 수 있다.
최우원리는 평균치라는 통계량과 연결되어 있다.


### 최우원리로 보는 베이즈 추정과 표준 통계학의 공통점

표준 통계학의 점추정에서 확률 L의 값을 살펴보기 위해 확률 p를 움직인다는 것은 베이즈 추정에서 현상이 일어나는 타입(원인)에 사전분포를 설정하고 그것을 변화시키는 것과 매우 흡사하다.
따라서 이 '**최우추정량이라는 사고법은 베이즈 추정의 발상과 공통되는 것**'이라고 이해할 수 있다.
